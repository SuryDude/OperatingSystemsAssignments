...
public class Schedule {
    ...
    /*
        private static int getNextNum(BufferedReader br) throws IOException {
            ...
            //tempChars.add(String.valueOf(temp));
            while (temp == ' '){
                temp = (char) br.read();
            }
    ...
    */
    ...
    public void input(String path) throws IOException {
        ...
        for (int i = 0; i < inputLine.length; i++){
            if (input[i].matches(".*[0-9].*") && inputLine[i].length() > 4) {
                String currentProcess = inputLine[i].toString();
                a = currentProcess[1];
            }
        }
        */
        ...
        int a, b, c, io;
        System.out.print("The original input was: " + cnt + brLine);
        int indexOFBRLine = 0;
        int[] letterArray = new int[4];
        int substringIndex = 0;
        for (int i = 0; i < cnt; i++) {
            //each parentheses group, depending on cnt being the first number on the line

            for (int j = 0; j < 4; j++) {
                //each a,b,c,io of the parentheses group
                String substringOFBRLine = brLine.substring(indexOFBRLine);
                Character subChar = substringOFBRLine.charAt(substringIndex);
                while (!Character.isDigit(subChar)) {
                    substringIndex++;
                    subChar = substringOFBRLine.charAt(substringIndex);
                }
                if (Character.isDigit(subChar)) {
                    //check if multidigit number
                    substringIndex++;
                    String multiDigitNum = "" + subChar;
                    while (Character.isDigit(substringOFBRLine.charAt(substringIndex))){
                        multiDigitNum+=substringOFBRLine.charAt(substringIndex);
                        substringIndex++;
                    }
                    letterArray[j] = Integer.parseInt(multiDigitNum);
    ...
    public static void main(String args[]) throws IOException {
        ...
        br = new BufferedReader(new FileReader("random-numbers.txt"));
        s.runFCFS();
        br.close();
        br = new BufferedReader(new FileReader("random-numbers.txt"));
        s.runRR();
